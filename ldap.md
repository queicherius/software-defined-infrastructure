...
Connecting anonymously to an exising server with [Apache Directory Studio](https://directory.apache.org/studio/) can be easily achieved by adding a new connection with:
...
Now, when seaching email adresses, we get asked for the user password and can see the emails in the LDAP server. We can also right-click on the new address book, choose "Properties" > "Offline" and "Download Now" to download all available data for offline use.
...
# The changetype of this file: changing an entry
changetype: modify
